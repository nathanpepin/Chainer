...
public readonly struct Result<T> : IResult<T>
{
    ...
    /// </summary>
    /// <value>
    ///     The <see cref="System.Exception" /> associated with the failure, or <c>null</c> if the operation was successful, or if the failure was not
    ...
    ///     Thrown if accessed when the result is in a failure state. The exception message includes the
    ///     <see cref="Error" />.
    /// </exception>
    /// <remarks>
    ...
    /// <returns>
    ///     A <see cref="Result{T}" /> instance indicating a failure, containing the error message and exception from the provided
    ...
    /// <summary>
    ///     Defines an implicit conversion from a tuple (<see cref="bool" /> Success, <see cref="string" /> Error, <typeparamref name="T" /> Value) to a
    ///     <see cref="Result{T}" />.
    ///     If `tuple.Success` is <c>true</c>, a successful <see cref="Result{T}" /> with `tuple.Value` is returned.
    ...
    /// <summary>
    ///     Defines an implicit conversion from a tuple (<see cref="bool" /> Success, <see cref="Exception" /> Exception, <typeparamref name="T" /> Value) to
    ///     a <see cref="Result{T}" />.
    ///     If `tuple.Success` is <c>true</c>, a successful <see cref="Result{T}" /> with `tuple.Value` is returned.
    ...
    /// <exception cref="InvalidOperationException">
    ///     Thrown if <see cref="IsSuccess" /> is <c>false</c> (i.e., an attempt is made to access the value of a
    ///     failed result).
    /// </exception>
    ...
    /// </summary>
    /// <param name="value">
    ///     When this method returns, contains the value of type <typeparamref name="T" /> if the result was successful, or the default value
    ///     of <typeparamref name="T" /> if it was a failure. This parameter is passed uninitialized.
    /// </param>
    ...
    /// </summary>
    /// <param name="error">
    ///     When this method returns, contains the error message if the result was a failure; otherwise, <c>null</c> (or
    ///     <see cref="string.Empty" /> if failure had no specific message but was not a success). This parameter is passed uninitialized.
    /// </param>
    ...
    ///     When this method returns, contains the <see cref="System.Exception" /> if the result was a failure and an exception was
    ///     provided; otherwise, <c>null</c>. This parameter is passed uninitialized.
    /// </param>
    ...
    /// </summary>
    /// <param name="value">
    ///     When this method returns <c>true</c>, contains the successful value. When it returns <c>false</c>, contains the default value of
    ///     <typeparamref name="T" />. This parameter is passed uninitialized.
    /// </param>
    /// <param name="error">
    ///     When this method returns <c>false</c>, contains the error message if the result was a failure. When it returns <c>true</c>,
    ///     contains <c>null</c> (or <see cref="string.Empty" />). This parameter is passed uninitialized.
    /// </param>
    /// <param name="exception">
    ///     When this method returns <c>false</c>, contains the <see cref="System.Exception" /> if the result was a failure and an
    ///     exception was provided. When it returns <c>true</c>, or if no exception was associated with the failure, contains <c>null</c>. This parameter is
    ...
    // Functional methods
    /// <summary>
    ///     If successful, transforms the encapsulated value of type <typeparamref name="T" /> to a new value of type <typeparamref name="TResult" /> using
    ///     the <paramref name="mapper" /> function.
    ...
    /// <returns>
    ///     A <see cref="Result{TResult}" /> which is successful with the mapped value if the original result was successful, or a failure propagating
    ...
    /// <summary>
    ///     If successful, asynchronously transforms the encapsulated value of type <typeparamref name="T" /> to a new value of type
    ///     <typeparamref name="TResult" /> using the asynchronous <paramref name="mapper" /> function.
    ///     If a failure, the failure (error message and exception) is propagated to the new <see cref="Result{TResult}" />.
    /// </summary>
    /// <typeparam name="TResult">The type of the value in the resulting <see cref="Result{TResult}" />.</typeparam>
    /// <param name="mapper">An asynchronous function to apply to the successful value.</param>
    /// <returns>
    ///     A <see cref="Task{T}" /> that will yield a <see cref="Result{TResult}" /> which is successful with the mapped value if the original result
    ///     was successful, or a failure propagating the original error details.
    /// </returns>
    ...
    /// <summary>
    ///     If the result is a failure, maps its <see cref="Error" /> message to a new error message using the specified <paramref name="errorMapper" />
    ///     function.
    ...
    /// <summary>
    ///     If the result is a failure, maps its <see cref="Exception" /> (if any) to a new exception using the specified <paramref name="exceptionMapper" />
    ///     function.
    ...
    /// <summary>
    ///     If successful, applies the <paramref name="binder" /> function to the encapsulated value, returning the <see cref="Result{TResult}" /> produced
    ///     by the binder.
    ///     If a failure, the failure (error message and exception) is propagated to the new <see cref="Result{TResult}" />.
    ...
    /// <returns>
    ///     The <see cref="Result{TResult}" /> returned by <paramref name="binder" /> if the original result was successful; otherwise, a failure
    ///     <see cref="Result{TResult}" /> propagating the original error details.
    /// </returns>
    ...
    /// <summary>
    ///     If successful, applies the asynchronous <paramref name="binder" /> function to the encapsulated value, returning the
    ///     <see cref="Result{TResult}" /> produced by the binder.
    ///     If a failure, the failure (error message and exception) is propagated to the new <see cref="Result{TResult}" />.
    ...
    /// <returns>
    ///     A <see cref="Task{T}" /> that will yield the <see cref="Result{TResult}" /> returned by <paramref name="binder" /> if the original result
    ///     was successful; otherwise, a failure <see cref="Result{TResult}" /> propagating the original error details.
    /// </returns>
    ...
    ///     If a failure, returns a <see cref="Results.Result.Failure(string)" /> or <see cref="Results.Result.Failure(Exception)" />, preserving the error
    ///     details.
    /// </summary>
    ...
    ///     If the <see cref="Result{T}" /> is successful but the value fails the <paramref name="predicate" />, a new failure <see cref="Result{T}" /> is
    ///     returned with the given <paramref name="error" /> message.
    /// </summary>
    ...
    /// <returns>
    ///     The original <see cref="Result{T}" /> if it's a failure or if it's successful and satisfies the <paramref name="predicate" />; otherwise, a
    ///     new failure <see cref="Result{T}" />.
    /// </returns>
    ...
    ///     If the <see cref="Result{T}" /> is already a failure, it's returned as is.
    ///     If the <see cref="Result{T}" /> is successful but the value fails the <paramref name="predicate" />, a new failure <see cref="Result{T}" /> is
    ///     returned, created from the given <paramref name="exception" />.
    /// </summary>
    /// <param name="predicate">The function to evaluate against the successful value. Must return <c>true</c> for the condition to be satisfied.</param>
    /// <param name="exception">
    ///     The exception to use for creating the failure <see cref="Result{T}" /> if the <paramref name="predicate" /> returns
    ///     <c>false</c>.
    /// </param>
    /// <returns>
    ///     The original <see cref="Result{T}" /> if it's a failure or if it's successful and satisfies the <paramref name="predicate" />; otherwise, a
    ...
    ///     If the <see cref="Result{T}" /> is already a failure, it's returned as is.
    ///     If the <see cref="Result{T}" /> is successful but the value fails the asynchronous <paramref name="predicate" />, a new failure
    ///     <see cref="Result{T}" /> is returned with the given <paramref name="error" /> message.
    /// </summary>
    /// <param name="predicate">
    ///     The asynchronous function (returning a <see cref="Task{Boolean}" />) to evaluate against the successful value. Must return
    ///     <c>true</c> for the condition to be satisfied.
    /// </param>
    /// <param name="error">The error message to use if the <paramref name="predicate" /> returns <c>false</c>.</param>
    /// <returns>
    ///     A <see cref="Task{T}" /> that will yield the original <see cref="Result{T}" /> if it's a failure or if it's successful and satisfies the
    ///     <paramref name="predicate" />; otherwise, a new failure <see cref="Result{T}" />.
    /// </returns>
    ...
    ///     If the <see cref="Result{T}" /> is already a failure, it's returned as is.
    ///     If the <see cref="Result{T}" /> is successful but the value fails the asynchronous <paramref name="predicate" />, a new failure
    ///     <see cref="Result{T}" /> is returned, created from the given <paramref name="exception" />.
    /// </summary>
    /// <param name="predicate">
    ///     The asynchronous function (returning a <see cref="Task{Boolean}" />) to evaluate against the successful value. Must return
    ///     <c>true</c> for the condition to be satisfied.
    ...
    ///     The action to execute if the <see cref="Result{T}" /> is a failure. It receives the error message and the (optional)
    ///     exception.
    /// </param>
    ...
    ///     The function to execute if the <see cref="Result{T}" /> is successful. It receives the successful value and returns a
    ///     <typeparamref name="TResult" />.
    /// </param>
    /// <param name="onFailure">
    ///     The function to execute if the <see cref="Result{T}" /> is a failure. It receives the error message and the (optional)
    ///     exception, and returns a <typeparamref name="TResult" />.
    /// </param>
    ...
    /// </summary>
    /// <param name="onSuccess">
    ///     The asynchronous action (returning a <see cref="Task" />) to execute if the <see cref="Result{T}" /> is successful. It
    ///     receives the successful value.
    /// </param>
    /// <param name="onFailure">
    ///     The asynchronous action (returning a <see cref="Task" />) to execute if the <see cref="Result{T}" /> is a failure. It
    ///     receives the error message and the (optional) exception.
    /// </param>
    ...
    /// <summary>
    ///     Asynchronously executes one of the provided asynchronous functions based on whether the <see cref="Result{T}" /> is a success or a failure, and
    ///     returns its result.
    /// </summary>
    /// <typeparam name="TResult">The type of the value to be returned by the match functions.</typeparam>
    /// <param name="onSuccess">
    ///     The asynchronous function (returning a <see cref="Task{TResult}" /> of <typeparamref name="TResult" />) to execute if the
    ///     <see cref="Result{T}" /> is successful. It receives the successful value.
    /// </param>
    /// <param name="onFailure">
    ///     The asynchronous function (returning a <see cref="Task{TResult}" /> of <typeparamref name="TResult" />) to execute if the
    ///     <see cref="Result{T}" /> is a failure. It receives the error message and the (optional) exception.
    /// </param>
    /// <returns>
    ///     A <see cref="Task{TResult}" /> that will yield the value returned by either the <paramref name="onSuccess" /> or
    ///     <paramref name="onFailure" /> asynchronous function.
    /// </returns>
    ...
    /// </param>
    /// <param name="onFailure">
    ///     The function to execute if the <see cref="Result{T}" /> is a failure. It receives the error message and returns a
    ///     <typeparamref name="TResult" />.
    /// </param>
    /// <returns>The value returned by either the <paramref name="onSuccess" /> or <paramref name="onFailure" /> function.</returns>
